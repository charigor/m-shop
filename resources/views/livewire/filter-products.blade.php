...
{{--                @endforeach--}}
                ...
                @if(isset($facet['facets']['brand']))
                    @foreach($facet['facets']['brand']['values'] as $key => $item)
...
{{--                        @foreach($item['values']['buckets'] as $k => $inner)--}}
                            ...
                            <div  class="z-10  transition-all bg-white ml-3" :class="!open ? 'invisible' : 'visible'" x-show.transition="open">
                                <ul class="text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                                    @foreach($item['values']['buckets'] as $m => $inner)
                                        <li class="px-1 ml-4">
                                            <div class="relative flex items-center">
                                                <input id="checkbox-item-{{$inner['key']}}" {{!$inner['doc_count']? 'disabled': '' }} wire:model.debounce.250ms="filter.{{$item['key']}}"
                                                                                                                              wire:model.debounce.250ms="{{$item['key']}}"
                                                       type="checkbox" value="{{$inner['key']}}"
                                                ...
                                                <label for="checkbox-item-{{$inner['key']}}" {{!$inner['doc_count'] ? 'disabled': '' }}
                                                class="disabled:opacity-25 flex font-semibold items-center p-[7px] hover:text-cyan-80 border-0 text-md leading-3 text-black">{{$inner['key']}}   <span class="ml-2 mb-[2px] text-xs text-cyan-600">{{$inner['doc_count']}}</span></label>
                        ...
                        ------------------------------------------------
                            {{--                                @foreach($inner['values']['buckets'] as $m => $mInner)--}}
...
{{--                        @endforeach--}}
                    @endforeach
                @endif
                    @if(isset($facet['facets']['all']))
                        @foreach($facet['facets']['all'] as $key => $item)
                            @foreach($item as $k => $inner)
                                        ...
                                        <div  class="z-10  transition-all bg-white ml-3" :class="!open ? 'invisible' : 'visible'" x-show.transition="open">
                                            <ul class="text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                                                @foreach($inner['values']['buckets'] as $m => $mInner)
                                                        <li class="px-1 ml-4">
                                                            <div class="relative flex items-center">
                                                                <input id="checkbox-item-{{$mInner['key']}}" {{!$mInner['doc_count']? 'disabled': '' }} wire:model.debounce.250ms="filter.{{$inner['key']}}"
{{--                                                                       wire:model.debounce.250ms="{{$inner['key']}}"--}}
                                                                       type="checkbox" value="{{$mInner['key']}}"
                                                                       class="w-4 h-4  disabled:opacity-25 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                                                <label for="checkbox-item-{{$mInner['key']}}" {{!$mInner['doc_count'] ? 'disabled': '' }}
                                                                class="disabled:opacity-25 flex font-semibold items-center p-[7px] hover:text-cyan-80 border-0 text-md leading-3 text-black">{{$mInner['key']}}   <span class="ml-2 mb-[2px] text-xs text-cyan-600">{{$mInner['doc_count']}}</span></label>
...
{{--                                     src="{{$product->getFirstMediaUrl('image')}}" alt="product image"/>--}}
                    ...
                    <div class="text-center m-[4px] relative">
                        ...
                        <div class="mt-2">
                            <button wire:click="addToCart({{$product['title']}})" type="button" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-1.5 mr-1 mb-1 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Add to cart</button>
                        </div>
                    </div>
                </div>
            @endforeach
