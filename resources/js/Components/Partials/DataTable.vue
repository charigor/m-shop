    ...
    <SectionMain>
        <div class="flex items-center justify-between mb-2 row">
            <div class="w-1/4 col-6">
                <input  type="search" @input="search" :value="params.search" aria-label="Search" placeholder="Search..." class="block w-full rounded-md border bg-gray-50 border w-full border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-50">
            </div>
        ...
        <table class=" table-auto w-full bg-white dark:bg-gray-700 shadow rounded">
            ...
            <tr class="text-left font-bold">
                ...
                <template v-for="column in columns">
                    <td v-if="column.value">
                        ...
                        <VueDatePicker class="dp__theme_dark" v-else-if="column.type === 'date'"  range  :partial-range="false"   multi-calendars v-model="params.filter[column.label]" :aria-label="column.label" ></VueDatePicker>
                    </td>
                </template>
                <td>
                    <template class="flex items-center">
                        <BaseButtons type="justify-start lg:justify-start" no-wrap>
                            ...
                            />
                            <div v-click-outside="() => {columnShow = false}" v-if="columnShow"  class="absolute z-10 bg-white rounded-lg shadow w-60 dark:bg-gray-700">
            ...
            <tr v-for="item in data.data" :key="data.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
                <td>
                    <label class="checkbox">
                ...
                <template v-for="column in columns" >
                    <td class="border-b-0 lg:w-6 before:hidden" v-if="column.value">
                        <Link class="flex items-center justify-center text-center px-6 py-4 focus:text-indigo-500" :href="`${urlPrefix}/${item.id}/edit`">
                            <div v-if="column.type === 'media'" class="center" style="width: 3rem">
                               <img style="width: 3rem" :src="item['media'].length ? item['media'][0]['preview_url']: ''" alt="">
                            </div>
                            <div v-else>
                                {{item[column.label] }}
                            </div>
        ...
        Are you sure you want to delete this users?
            <ul><li v-for="item in selected"><b>{{item.id}}.{{item.email}}</b></li></ul>
        </CardBoxModal>
    </SectionMain>
