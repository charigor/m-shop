...
const form = reactive({
    group_type: props.attribute_group.group_type,
    lang: {},
    _method: 'put'
});
for(let item in usePage().props.languages){
    let code = usePage().props.languages[item]['code'];
    if(props.attribute_group.translation.hasOwnProperty(code)){
        form['lang'][code] = props.attribute_group.translation[code]
    }else{
        form['lang'][code] =  {
            name:'',
            public_name:'',
...
const submit = () => {
    router.put(`${urlPrefix}/${props.attribute_group.id}`, form, {preserveState: true})
}
defineComponent({
    LayoutAuthenticated,
    ...
    <LayoutAuthenticated>
        <SectionMain>
            <SectionTitleLineWithButton
                ...
                :title="$t('page.attribute_group.title_edit')"
                main
            >
            </SectionTitleLineWithButton>
            ...
            <form @submit.prevent="submit()" class="mb-4 p-8 border border-gray-200 dark:border-gray-700">
                ...
                <div class="relative  z-0 w-96 mb-6 group required">
                    <div  class="relative z-0 h-6 w-full group">
                    ...
                    <select v-model="form.group_type" aria-label="group_type" class="block  w-full rounded-md border bg-gray-50 border w-full  border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-50">
