...
const form = reactive({
    guard_name: props.feature.guard_name,
    lang: {},
    _method: 'put'
});
for(let item in usePage().props.languages){
    let code = usePage().props.languages[item]['code'];
    if(props.feature.translation.hasOwnProperty(code)){
        form['lang'][code] = props.feature.translation[code]
    }else{
        form['lang'][code] =  {
            name:''
        }
...
const submit = () => {
    router.put(`${urlPrefix}/${props.feature.id}`, form, {preserveState: true})
}
defineComponent({
    LayoutAuthenticated,
    ...
    <LayoutAuthenticated>
        <SectionMain>
            <SectionTitleLineWithButton
                ...
                :title="$t('page.feature.title_edit')"
                main
            >
            </SectionTitleLineWithButton>
            ...
            <form @submit.prevent="submit()" class="mb-4 p-8 border border-gray-200 dark:border-gray-700">
                <div class="relative z-0 w-full mb-6 group required">
                    <input v-model="form['guard_name']"  type="text" name="guard_name" id="guard_name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" />
                    <label for="guard_name" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Guard name</label>
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500" v-if="$page.props.errors.guard_name">{{$page.props.errors.guard_name}}</p>
                </div>
