...
const form = reactive({
    ...
    active: props.brand.active,
    image: props.brand.image.map(i => i.file_name),
    lang: {},
    _method: 'put'
});
for(let item in usePage().props.languages){
    let code = usePage().props.languages[item]['code'];
    if(props.brand.translation.hasOwnProperty(code)){
        form['lang'][code] = props.brand.translation[code]
    }else{
        form['lang'][code] =  {
            short_description:'',
            description:'',
