    ...
    <LayoutAuthenticated>
        <SectionMain>
            ...
            <form @submit.prevent="submit()" class="mb-4 p-8 border border-gray-200 dark:border-gray-700">
                ...
                <DraggableVueDropzone :w="Number(250)" :h="Number(250)" @updatePosition="(files) => form.image = files"  @mainImage="(file) => form.main_image = file" :maxFiles="Number(3)"  multiple="1"  @removeImage="(file) => form.image = form.image.filter((item) => item !== file)" @loadImages="(file) => form.image.push(file)" path="/admin/product/storeMedia" :files="[]"></DraggableVueDropzone>
                </div>
