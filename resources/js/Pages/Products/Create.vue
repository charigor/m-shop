...
import {defineComponent, defineProps, ref, onMounted, reactive, computed, watch} from "vue";
...
console.log(form.unit_price_ratio)
console.log(form.price)
const tempUnity = ref(0)
      ...
      <div class="flex justify-between">
        <ul class="flex flex-wrap  justify-start text-sm font-medium text-center mb-1" role="tablist">
          <li role="product">
            ...
            <button type="button" @click="tabPart('#type')" v-if="form.type ==='2'"
      ...
      <form @submit.prevent="submit()" class="mb-4 p-8 border border-gray-200 dark:border-gray-700">
        ...
        <div class="grid grid-cols-4 gap-10">
          ...
          <template v-if="activeTab ===  '#main' || activeTab === ''">
            <div class="col-span-3">
              <div v-for="language in $page.props.languages" :key="language.id">
                <div class="relative z-0 w-full mb-6 group required" v-show="locale === language.code">
                  <input v-model="form['lang'][language.code]['name']" @input="changeSlug(language.code)" type="text"
              ...
              <div class="relative w-full mb-7 z-10 group">
                ...
                <div class="relative row mb-6 group pt-7">
                  <DraggableVueDropzone :w="Number(130)" :h="Number(115)"
                                        @updatePosition="(files) => form.image = files" :main_image="form.main_image"
                                        @mainImage="(file) => form.main_image = file" :maxFiles="Number(15)"
                                        multiple="1"
                ...
                <div class="relative w-full z-10 group" v-show="locale === language.code">
                  ...
                  <template v-for="language in $page.props.languages" :key="language.id">
                    <div class="relative z-0 w-full mb-6 pt-7 group"
                      ...
                      <CKEditor v-model="form['lang'][language.code]['short_description']" :lang="language.code"
                                :csrf="$page.props.csrf_token"/>
                    </div>
                    <div class="relative z-0 w-full mb-6 pt-7 group"
                         v-if="activeTabDesc === `description_${language.code}`">
                      <CKEditor v-model="form['lang'][language.code]['description']" :lang="language.code"
                                :csrf="$page.props.csrf_token"/>
                    </div>
                  </template>
                </div>
            ...
            <div class="col-span-1">
              <div class="p-3">
                ...
                <Radio :label="$t(`page.${pageName}.combo.simple_product`)" checked
                       value="regular" v-model="form.type"/>
                <Radio :label="$t(`page.${pageName}.combo.combo_product`)" checked
              ...
              <div class="p-3">
                ...
                <div class="relative z-0 w-full mb-6 group required">
                  <input v-model="form.reference" type="text"  id="reference"
                         class="block indent-2 py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"/>
                  <p class="mt-2 text-sm text-red-600 dark:text-red-500" v-if="$page.props.errors.reference">
              ...
              <div class="p-3">
                ...
                <div class="relative z-1 w-full mb-6 group">
                  <input v-model="form.price" @input="inputPrice" @focus="(e) => e.target.value === String(0) ? e.target.value = null : ''" type="text" id="price_main"
                  ...
                  <p class="mt-2 text-sm text-red-600 dark:text-red-500" v-if="$page.props.errors.price">
                    {{ $page.props.errors.price }}</p>
                  <input v-model="taxPrice" @input="inputTaxPrice"  @focus="(e) => e.target.value === String(0)? e.target.value = null : ''" type="text" id="price_tax"
                         class="mb-2 block indent-2 py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"/>
              ...
              <div class="p-3">
                ...
                <div class="relative z-0 w-full mb-6 group required">
                  <input v-model="form.quantity" type="number" min="0" id="quantity_main"
                         class="block indent-2 py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"/>
                  <p class="mt-2 text-sm text-red-600 dark:text-red-500" v-if="$page.props.errors.quantity">
              ...
              <div class="p-3">
                ...
                <template v-for="category in props.categories.data" :key="category.id">
                  <Checkbox :style="{'margin-left': `${1 * category.depth}rem`}"
                            :label="category.translation[locale].title" v-model="form.categories" :value="category.id"/>
                </template>
              </div>
            </div>
          </template>
          <template v-if="activeTab === '#price'">
            <div class="col-span-4">
              <div class="grid grid-cols-5 gap-10">
                <div class="col-span-1">
                  <div class="relative z-0 w-full mb-6 group">
                    <Label class="text-lg mb-2" for="tab_price" :content="$t(`page.${pageName}.blocks.price.fields.price`)"/>
                    <input v-model="form.price" @input="inputPrice" type="text" id="tab_price" @focus="(e) => e.target.value === String(0) ? e.target.value = null : ''"
                           class="mb-2 block indent-2 py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"/>
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500" v-if="$page.props.errors.price">
                      {{ $page.props.errors.price }}</p>
                  </div>
                ...
                <div class="col-span-1">
                  <div class="relative z-0 w-full mb-6 group">
                    <Label class="text-lg mb-2" for="tab_price_tax" :content="$t(`page.${pageName}.blocks.price.fields.price_with_tax`)"/>
                    <input v-model="taxPrice" @input="inputTaxPrice" type="text" id="tab_price_tax" @focus="(e) => e.target.value === String(0) ? e.target.value = null : ''"
                           class="mb-2 block indent-2 py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"/>
                  </div>
                </div>
                <div class="col-span-1">
                  <div class="relative z-0 w-full mb-6 group">
                    <Label class="text-lg mb-2" for="tab_tax" :content="$t(`page.${pageName}.blocks.price.fields.tax`)"/>
                    <Multiselect class="multiselect-dark-theme mb-8  z-10"
                                 mode="single"
                                 label="name"
                                 valueProp="id"
              ...
              <div class="grid grid-cols-5 gap-10">
                <div class="col-span-1">
                  <div class="relative z-0 w-full mb-6 group">
                    <Label class="text-lg mb-2" for="unit_price_ratio" :content="$t(`page.${pageName}.blocks.price.fields.unit_price_ratio`)"/>
                ...
                <div class="col-span-1">
                  <div class="relative z-0 w-full mb-6 group">
                    <Label class="text-lg mb-2" for="unity" :content="$t(`page.${pageName}.blocks.price.fields.unity`)"/>
                    <input v-model="form.unity" type="text" id="unity" :placeholder="$t(`page.${pageName}.blocks.price.placeholders.unity`)"
                           class="mb-2 block indent-2 py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"/>
              ...
              <div v-for="language in $page.props.languages" :key="language.id">
                <div class="relative z-0 w-full mb-6 group" v-show="locale === language.code">
                  <input v-model="form['lang'][language.code]['link_rewrite']" type="text"
                         :name="`${language.code}link_rewrite`" :id="`${language.code}link_rewrite`"
                         class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"/>
                ...
                <div class="relative z-0 w-full mb-6 group" v-show="locale === language.code">
                  <input v-model="form['lang'][language.code]['meta_description']" type="text"
                         :name="`${language.code}meta_description`" :id="`${language.code}meta_description`"
                         class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"/>
          ...
          <template v-if="activeTab === '#type'">
            <Combinations :attributes="props.attributes" @addNewAttribute="(e) => addAttribute(e.value)"></Combinations>
          </template>
        </div>
